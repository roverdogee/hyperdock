...
nonisolated final class AXCore: @unchecked Sendable {
    ...
    /// Window elements resolved by a route other than the application's window list,
    /// remembered so a later write can address the same window directly.
    ///
    ...
    /// element, so keeping it is the difference between a write that lands and one that
    /// silently does nothing — which is how dragging and snapping came to work on one
    /// window of such an application and no others.
    ///
    ...
    /// A window identified by hit-testing a screen point.
    nonisolated struct HitWindow: Sendable {
        let id: CGWindowID
        let pid: pid_t
        /// Global, top-left origin coordinates, as Accessibility reports them.
        let frame: CGRect
    }

    /// The frontmost application's focused window.
    ///
    /// This is what keyboard commands must act on. Resolving the target from the pointer
    /// instead only works for the first keystroke: once the window has been moved it is
    /// no longer under the (stationary) pointer, so every following press finds a
    /// different window or nothing at all.
    ///
    /// `kAXFocusedWindowAttribute` is used rather than `kAXWindowsAttribute` because the
    /// latter is filtered to the current Space, while the focused window is always
    /// reachable — and it is by definition on screen.
    func focusedWindow() async -> HitWindow? {
        ...
        return await perform {
            ...
            guard let raw = Self.value(app, kAXFocusedWindowAttribute),
                  let element = Self.checkedElement(raw) else { return nil }

            var windowID: CGWindowID = 0
            guard _AXUIElementGetWindow(element, &windowID) == .success, windowID != 0 else {
                return nil
            }
            self.remember(element, id: windowID, pid: pid)
            ...
            if let raw = Self.value(element, kAXSizeAttribute),
               let value = Self.checkedValue(raw) {
                AXValueGetValue(value, .cgSize, &size)
            }
            return HitWindow(id: windowID, pid: pid, frame: CGRect(origin: origin, size: size))
        }
    }

    /// The window under a point, given in Accessibility (top-left origin) coordinates.
    ///
    /// Uses the system-wide element's hit test and then walks up to the enclosing
    /// window, which is what makes "grab anywhere in the window" work rather than only
    /// on the title bar.
    func window(at point: CGPoint) async -> HitWindow? {
        await perform {
            ...
            // Walk up until we reach an AXWindow, bounded so a malformed hierarchy
            // cannot spin here.
            var depth = 0
            while depth < 12 {
                if Self.value(element, kAXRoleAttribute) as? String == kAXWindowRole {
                    break
                }
            ...
            guard Self.value(element, kAXRoleAttribute) as? String == kAXWindowRole else {
                return nil
            }
            ...
            // Never our own windows. The preview bubble sits at `.popUpMenu` level under
            // the pointer for as long as it is open, so a hit test aimed at "the window
            // the user is pointing at" resolves *it* — and every caller then acts on the
            // bubble. Measured: with title-bar scrolling on, a scroll over the bubble's
            // header (its top 28 pt, which is exactly where the application name sits)
            // made HyperDock snap its own panel, which reads as the preview reacting to a
            // click. The same hole let a modifier-drag pick the bubble up.
    ...
    /// Moves and resizes a window. `frame` is in Accessibility (top-left origin) space.
    ///
    /// Returns the frame the window actually ended up with, which is not always the one
    /// asked for: setting `kAXSizeAttribute` is advisory and applications clamp it —
    /// measured here, a request for 50×20 produced 87×33. Callers that care must use the
    /// returned value rather than assume.
    @discardableResult
    func setFrame(_ frame: CGRect, window id: CGWindowID, pid: pid_t) async -> CGRect? {
        await perform {
            guard let element = self.findWindow(id: id, pid: pid) else { return nil }
            return self.applyFrame(frame, to: element, pid: pid)
        }
    }

    /// Moves and resizes whichever window the given application has focused.
    ///
    /// Resolves the target through `kAXFocusedWindowAttribute` instead of searching
    /// `kAXWindowsAttribute` for a window id, and that is the whole point of it existing.
    /// Measured on this machine: Sublime Text reports seven windows to CGWindowList and
    /// exactly **one** to `kAXWindowsAttribute`; WeChat, Surge Dashboard and Chrome Canary
    /// do not list their focused window there at all. Looking a window up by id therefore
    /// finds nothing for every window but the one the application chose to advertise, and
    /// the caller's write silently does nothing — which is exactly how keyboard snapping
    /// came to work on one window of an application and no others.
    ///
    /// The focused-window attribute has no such gap: it is answered directly by the
    /// application, and it is what the user means by "this window" anyway.
    @discardableResult
    func setFrameOnFocusedWindow(_ frame: CGRect, pid: pid_t) async -> CGRect? {
        await perform {
            let app = self.cachedApplication(pid)
            guard let raw = Self.value(app, kAXFocusedWindowAttribute),
                  let element = Self.checkedElement(raw) else { return nil }
            var id: CGWindowID = 0
            if _AXUIElementGetWindow(element, &id) == .success { self.remember(element, id: id, pid: pid) }
            return self.applyFrame(frame, to: element, pid: pid)
        }
    }

    /// The frame write itself. Must be called on `queue`.
    private nonisolated func applyFrame(_ frame: CGRect,
                                        to element: AXUIElement,
                                        pid: pid_t) -> CGRect? {
        dispatchPrecondition(condition: .onQueue(queue))

        // Some applications advertise AXEnhancedUserInterface (a VoiceOver
        // compatibility flag). While it is set, kAXPosition writes are silently
        // dropped — the call returns success and the window does not move. Clearing
        // it for the duration of the write is the standard workaround.
        let app = self.cachedApplication(pid)
        let enhanced = Self.value(app, "AXEnhancedUserInterface") as? Bool ?? false
        if enhanced {
        ...
        // Size, then position, then size again. Moving across displays of different
        // sizes can clamp an over-large window on the way, so the second size pass
        // reasserts the intent once the window is on the destination screen.
        ...
        if let raw = Self.value(element, kAXPositionAttribute),
           let value = Self.checkedValue(raw) {
            AXValueGetValue(value, .cgPoint, &appliedOrigin)
        }
        if let raw = Self.value(element, kAXSizeAttribute),
           let value = Self.checkedValue(raw) {
            AXValueGetValue(value, .cgSize, &appliedSize)
        }
